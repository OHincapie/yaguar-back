...
async def create_supplier(_: CurrentUser, data: SupplierCreate, service: Annotated[SupplierService, Depends(get_service)]):
    return await service.create_supplier(data)


@router.get("/{id}", response_model=SupplierRead)
async def get_supplier(_: CurrentUser, id: str, service: Annotated[SupplierService, Depends(get_service)]):
    return await service.get_supplier(id)


@router.put("/{id}", response_model=SupplierRead)
async def update_supplier(_: CurrentUser, id: str, data: SupplierUpdate, service: Annotated[SupplierService, Depends(get_service)]):
    return await service.update_supplier(id, data)


@router.delete("/{id}", response_model=MessageResponse)
async def delete_supplier(_: CurrentUser, id: str, service: Annotated[SupplierService, Depends(get_service)]):
